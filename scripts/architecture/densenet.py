# Adapted from CIFAR-ZOO repository
# -*-coding:utf-8-*-
import math
...
class DenseNet(nn.Module):
    def __init__(self, block, depth, growth_rate=12, reduction=0.5, num_classes=10, input_channel=3):
        super(DenseNet, self).__init__()
        self.growth_rate = growth_rate

        nblocks = (depth - 4) // 6
        num_planes = 2 * growth_rate
        self.conv_1 = nn.Conv2d(input_channel, num_planes, kernel_size=3, padding=1, bias=False)
    ...
    def forward(self, x, hook=False):
        ...
        if hook:
            return  {'pred': out.detach().cpu().numpy(), 'x_flat': out_flat.detach().cpu().numpy()}
        else:
            return out


def densenet100bc(num_classes, input_channel=3):
    return DenseNet(Bottleneck, depth=100, growth_rate=12, num_classes=num_classes, input_channel=input_channel)


def densenet190bc(num_classes, input_channel=3):
    return DenseNet(Bottleneck, depth=190, growth_rate=40, num_classes=num_classes, input_channel=input_channel)
